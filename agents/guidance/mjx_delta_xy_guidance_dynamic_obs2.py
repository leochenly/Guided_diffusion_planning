...
def min_obstacle_clearance_xy(points_xy: jp.ndarray,
                             centers_xy: jp.ndarray,
                             radii: jp.ndarray,
                             safety_margin: float) -> jp.ndarray:
    """Return min(dist - (r+margin)) over all points & obstacles. Negative => penetration."""
    if centers_xy is None or radii is None:
        return jp.asarray(1e9, dtype=jp.float32)
    diff = points_xy[:, None, :] - centers_xy[None, :, :]
    dist = jp.linalg.norm(diff, axis=-1)
    safe_r = (radii + safety_margin)[None, :]
    clearance = dist - safe_r
    return jp.min(clearance)
...
def obstacle_penalty_xy(points_xy: jp.ndarray,
    ...
    if centers is None or radii is None:
        print("WARNING: No obstacles provided. Returning 0.0 for obstacle cost.")
        return jp.array(0.0, dtype=jp.float32)  # 返回有效的数字，而不是 None
    # 添加计算障碍物惩罚的代码（例如计算与障碍物的距离等）

    # 如果有障碍物，继续计算惩罚
    # 这里的逻辑可以根据需求进行调整
    penalty = 0.0
    # 计算障碍物惩罚
    return jp.array(penalty, dtype=jp.float32)
...
class MJXDeltaXYGuidance:
    ...
    def set_context(
        ...
        *,
        ...
        obstacles_centers_xy: Optional[np.ndarray | jp.ndarray] = None,
        obstacles_radii: Optional[np.ndarray | jp.ndarray] = None,
        safety_margin: Optional[float] = None,
    ):
        self._data0 = data0
        self._xy0 = jp.array(np.asarray(current_xy, dtype=np.float32))
        self._xy_ref = jp.array(np.asarray(xy_ref, dtype=np.float32)) if xy_ref is not None else None
    ...
    def _loss_fn(self, dxy_seq: jp.ndarray):
        ...
        # 添加调试输出，查看 obs_cost
        print(f"obs_cost: {obs_cost}")

        # for debug / logging: minimum clearance (dist - (r+margin)) over the MJX rollout
        min_clearance = min_obstacle_clearance_xy(
            ee_real[:, :2],
            self._obs_centers,
            self._obs_radii,
            safety_margin=self.safety_margin,
