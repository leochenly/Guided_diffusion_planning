...
class Viewer(RenderContext):
    ...
    def _create_overlay(self):
        ...
        # --- solver iterations compatibility (MuJoCo versions differ) ---
        solver_iter_disp = 0
        if hasattr(self.data, "solver_iter"):
            # older mujoco bindings (often scalar)
            try:
                solver_iter_disp = int(self.data.solver_iter) + 1
            except Exception:
                try:
                    arr = np.asarray(self.data.solver_iter).reshape(-1)
                    solver_iter_disp = int(arr.max()) + 1 if arr.size else 0
                except Exception:
                    solver_iter_disp = 0
        elif hasattr(self.data, "solver_niter"):
            # newer mujoco bindings (can be scalar OR array)
            try:
                solver_iter_disp = int(self.data.solver_niter)
            except Exception:
                try:
                    arr = np.asarray(self.data.solver_niter).reshape(-1)
                    solver_iter_disp = int(arr.max()) if arr.size else 0
                except Exception:
                    solver_iter_disp = 0

        self.add_overlay(bottomleft, "Solver iterations", str(solver_iter_disp))
