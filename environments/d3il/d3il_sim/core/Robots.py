...
class RobotBase:
    ...
    def __init__(self, scene, dt, num_DoF=7, base_position=None, base_orientation=None):
        ...
        if getattr(scene, "use_mjx", False):
            self.cartesianPosQuatTrackingController = ctrl.CartPosQuatImpedenceControllerMJX()
        else:
            self.cartesianPosQuatTrackingController = ctrl.CartPosQuatImpedenceController()
    ...
    def preprocessCommand(self, target_j_acc):
        """
        JAX-friendly preprocess:
        - uses jnp for all math
        - keeps semantics identical to old numpy version
        """
        ...
        # Use jnp arrays
        tj = jnp.asarray(target_j_acc)

        # finger commands (still computed from current_fing_pos/vel)
        # you can later JAX-ify fing_ctrl_step too; for now keep consistent by converting output.
        finger = jnp.asarray(self.fing_ctrl_step())

        tj9 = jnp.concatenate([tj, finger], axis=0)

        if self.use_inv_dyn:
            self.des_joint_acc = np.array(tj9)
            uff = self.get_command_from_inverse_dynamics(tj9, mj_calc_inv=True)
            uff = uff.at[7:9].set(finger)
        else:
            uff = tj9
            if self.gravity_comp:
                comp = self.get_command_from_inverse_dynamics(jnp.zeros((9,)), mj_calc_inv=False)
                self.grav_terms = np.array(comp)
                uff = uff + comp
        ...
        # rate limit
        if self.clip_rate:
            uff_last = jnp.asarray(self.uff_last)
            uff_diff = uff - uff_last
            uff_diff = jnp.clip(uff_diff, -self.rate_limit, self.rate_limit)
            uff = uff_last + uff_diff

        # torque clip
        if self.clip_actions:
            torque_limit = jnp.asarray(self.torque_limit)
            uff = uff.at[:7].set(jnp.clip(uff[:7], -torque_limit, torque_limit))

        # store (numpy for legacy chain)
        self.uff = np.array(uff)
        self.uff_last = np.array(uff)
        self.finger_commands = np.array(finger)
